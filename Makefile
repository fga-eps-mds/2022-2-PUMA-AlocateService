...
test:
	...
	(./tests/utils/wait-for-it-test.sh db-alocate-test:5432 -- true && echo "\033[92mInserting data to database... \033[0m" && \
	...
	sudo docker-compose -f test.docker-compose.yaml exec -T alocate-service-test npm run test);\
...
test-debug:
	...
	./tests/utils/db_script_test.sh drop && ./tests/utils/db_script_test.sh create && ./tests/utils/db_script_test.sh populate);\
	echo "\033[96mRunning Tests...\033[0m" && \
	sudo docker-compose -f test.docker-compose.yaml exec -T alocate-service-test npm run test-debug);\
	sudo docker-compose -f test.docker-compose.yaml down
